...
class QLearning():
    def train(self, env: Environment):
        ...
        for episode in range(1, n_episodes + 1):
            ...
            while not done:
                action_index = eps_greedy(rng, Q[state], epsilon)
                next_state, reward, done = env.step(action_index)
                Q[state, action_index] = Q[state, action_index] + alpha * (reward + gamma * max(Q[next_state, :]) - Q[state, action_index])
                state = next_state
                if done:
                    break
